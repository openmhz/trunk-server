...
*/
  //  url = startUp.backend_server + "/" + startUp.shortName.toLowerCase() + "/calls" + url + '?' + buildUrlParams(params);
    url = backend_server + "/" + state.call.shortName + "/calls" + url + '?' + buildUrlParams(params);
...
export function addStar( callId) {
  return (dispatch, getState) => {
    ...
    const url = backend_server + "/" + state.call.shortName.toLowerCase() + "/star/" + callId
    return axios
      .post(url,{}, {
        headers: {
...
export function fetchCallInfo( callId) {
  return (dispatch, getState) => {
    ...
    const url = backend_server + "/" + state.call.shortName.toLowerCase() + "/call/" + callId
    return axios
      .get(url)
      .then(response => {
        if (response.data.success) {
